...
public class UsersDao extends PersonsDao {
	...
	public Users create(Users user) throws SQLException {
		...
		create(new Persons(user.getUserName(), user.getPassword(), user.getFirstName(), user.getLastName(),
	...
	public Users getUserFromUserName(String userName) throws SQLException {
		...
		String selectUser = "SELECT Users.UserName AS UserName, Password, FirstName, LastName, Email, PhoneNumber, DoB, Subscribed "
	...
	public List<Users> getAllUsers() throws SQLException {
		...
		try {
			...
			while (results.next()) {
				...
				boolean subscribed = results.getBoolean("Subscribed");
				users.add(
						new Users(userName, password, resultFirstName, lastName, email, phoneNumber, dob, subscribed));
			}
		} catch (SQLException e) {
			e.printStackTrace();
			throw e;
	...
	public List<Users> getUserFromSubscribed(boolean isSubscribed) throws SQLException {
		...
		try {
			...
			while (results.next()) {
				...
				boolean subscribed = results.getBoolean("Subscribed");
				users.add(
						new Users(resultUserName, password, firstName, lastName, email, phoneNumber, dob, subscribed));
			}
		} catch (SQLException e) {
			e.printStackTrace();
			throw e;
	...
	/**
	 * Updates any changed information of the User instance. This runs a UPDATE
	 * statement. It can only update Dob and Subscribed.
	 */
	public Users updateUser(Users user) throws SQLException {
		String updateUser = "UPDATE Users SET DoB=?, Subscribed=? WHERE UserName=?;";
		Connection connection = null;
		PreparedStatement updateStmt = null;
		try {
			connection = connectionManager.getConnection();
			updateStmt = connection.prepareStatement(updateUser);
			updateStmt.setTimestamp(1, new Timestamp(user.getDob().getTime()));
			updateStmt.setBoolean(2, user.isSubscribed());
